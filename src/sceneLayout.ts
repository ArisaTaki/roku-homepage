...
const LAST_WORK_LEFT = 6030;
const LAST_WORK_WIDTH = 580;
...
export function getSceneLayout(width: number, height: number) {
  ...
  const travel = LAST_WORK_LEFT + workOffset + LAST_WORK_WIDTH / 2 - viewportWidth / 2;
  ...
  return {
    ...
    trackWidth: Math.max(LAST_WORK_LEFT + workOffset + LAST_WORK_WIDTH, viewportWidth),
  };
}
